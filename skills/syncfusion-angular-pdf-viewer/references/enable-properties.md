# Enable Properties in Angular Syncfusion PDFViewer Component
...
The Angular PDFViewer component uses boolean enable properties to control feature availability. Each property acts as a feature toggle, allowing you to build specialized viewers for different use cases.
...
| **enableDesktopMode** | Enables or disables desktop mode for optimized viewing experience on desktop devices. When enabled on mobile, provides desktop-style toolbar. | `boolean` |
...
| **enablePrint** | Enables or disables the print functionality, allowing users to print the PDF document. Print is unavailable in mobile mode unless enableDesktopMode is true. | `boolean` |
...
| **enableTextSelection** | Enables or disables text selection in the PDF document for copying and highlighting. Set to false on mobile when enableDesktopMode is true to improve scrolling. | `boolean` |
...
**When to use**: Enable for standard viewing. Disable `enableTextSelection` for security-restricted documents or to improve mobile scrolling when desktop mode is enabled.
...
- `enablePrint` - Print document (unavailable in mobile mode unless enableDesktopMode is true)
...
- `enableDesktopMode` - Desktop-optimized rendering (enables desktop toolbar on mobile devices)

**When to use**: Enable persistence for better UX. Disable local storage for security-sensitive applications. Set enableDesktopMode to false for mobile-optimized interface.
...
- Disable `[enablePageOrganizer]="false"` and `[enableFormDesigner]="false"` if not needed
...
### Issue: Print option not available on mobile
**Cause**: Print functionality is unavailable in mobile mode by default
**Solution**: Set `[enableDesktopMode]="true"` to enable desktop toolbar with print option on mobile devices

### Issue: Scrolling issues on mobile with desktop mode
**Cause**: Text selection interferes with touch scrolling when desktop toolbar is enabled
**Solution**: Set `[enableTextSelection]="false"` to improve touch-based scrolling

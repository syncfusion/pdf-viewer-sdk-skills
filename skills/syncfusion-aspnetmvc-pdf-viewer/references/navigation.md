...
```cshtml
@Html.EJS().PdfViewer("pdfviewer").EnableNavigation(true).Render()
```
...
```cshtml
@Html.EJS().PdfViewer("pdfviewer").EnableNavigation(false).Render()
```
...
```cshtml
@Html.EJS().PdfViewer("pdfviewer").EnableThumbnail(false).Render()
```
...
```cshtml
@Html.EJS().PdfViewer("pdfviewer").EnableHyperlink(false).Render()
```
...
@Html.EJS().PdfViewer("pdfviewer").EnableHyperlink(false).HyperlinkOpenState(Syncfusion.EJ2.PdfViewer.LinkTarget.NewTab).Render()
```

```cshtml
<!-- Open links in same window (default) -->
@Html.EJS().PdfViewer("pdfviewer").EnableHyperlink(false).HyperlinkOpenState(Syncfusion.EJ2.PdfViewer.LinkTarget.CurrentTab).Render()
```
...
```cshtml
@Html.EJS().PdfViewer("pdfviewer").EnableToolbar(true).EnableNavigation(true).EnableBookmark(true).EnableThumbnail(true).EnableHyperlink(true).Render()
```
...
```cshtml
@Html.EJS().PdfViewer("pdfviewer").EnableFormFields(false).EnableNavigation(true).EnableBookmark(true).EnableAnnotation(false).EnableHyperlink(true).Render()
```

...
```cshtml
@Html.EJS().PdfViewer("pdfviewer").ZoomValue(150).PageCount(10).Width("100%").Height("100%").Locale("en-US").Render()
```
...
@{
    ...
    var commandManagerConfigJson =
        JsonConvert.SerializeObject(commandManagerConfig);
}

@Html.EJS().PdfViewer("pdfviewer").CommandManager(@Html.Raw(commandManagerConfigJson)).Render()
```

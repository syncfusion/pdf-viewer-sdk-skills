...
These properties are available directly on the `PdfViewer` to control context menu-related functionality:
...
viewer.contextMenuOption = 'None';  // Disables the entire context menu
viewer.disableContextMenuItems = ['cut', 'paste'];  // Disables specific items
```
---
...
The PDF Viewer supports adding custom options to the context menu using the `addCustomMenu()` method. Define actions for custom items with the `customContextMenuSelect()` event method.
The context menu dynamically adapts based on what element the user right-clicks, providing context-aware options for text selections, annotations, form fields, and empty space.
        ...
        if (menuElement) {
            ...
            // Show "Lock" only for unlocked annotations, "Unlock" only for locked ones
                ...
                for (let j = 0; j < viewer.selectedItems.annotations.length; j++) {
                    const selectedAnnotation: any = viewer.selectedItems.annotations[j];
...
```typescript
let viewer: PdfViewer = new PdfViewer();
viewer.contextMenuOption = 'None'; // Completely disables the context menu
```
---
...
viewer.customContextMenuBeforeOpen = function (args: any) {
    for (let i = 0; i < args.ids.length; i++) {
        const search = document.getElementById(args.ids[i]);
        if (search) {
            ...
            if (args.ids[i] === 'search_in_google' && viewer.textSelectionModule 
                ...
                for (let k = 0; k < viewer.selectedItems.formFields.length; k++) {
                    const selectedFormFields = viewer.selectedItems.formFields[k];
                    if (selectedFormFields) {
                        const selectedFormField = viewer.selectedItems.formFields[k].isReadonly;
                        const displayMenu = (isReadOnlyOption && !selectedFormField) || 
                            (!isReadOnlyOption && selectedFormField);
                        search.style.display = displayMenu ? 'block' : 'none';
                    }
                }
            } 
            else if (args.ids[i] === 'formfield properties' 
                && viewer.selectedItems.formFields.length !== 0) {
                search.style.display = 'block';
            }
        }
...
function setReadOnlyTrue(args: any): void {
...
function setReadOnlyFalse(args: any): void {
    const selectedFormFields = viewer.selectedItems.formFields;
    for (let i = 0; i < selectedFormFields.length; i++) {
        const selectedFormField = selectedFormFields[i];
        if (selectedFormField) {

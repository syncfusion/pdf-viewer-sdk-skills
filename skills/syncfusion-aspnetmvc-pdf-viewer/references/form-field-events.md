...
Brief: The Syncfusion ASP.NET MVC PDF Viewer provides a comprehensive set of form field events that report changes associated with creating, selecting, modifying, moving, resizing, or removing form fields. These events supply metadata related to the affected field and are raised during user interaction or programmatic updates.
...
| **formFieldFocusOut** | Triggered when a form field loses focus after editing. | **formFieldFocusOutEventArgs** | **fieldName** - (FormFieldInfo) - Gets the form field object that lost focus. **pageIndex** - (number) - Gets the page index where the form field lost focus. |

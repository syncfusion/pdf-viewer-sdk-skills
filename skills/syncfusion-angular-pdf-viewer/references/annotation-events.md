...
- **Event Name**: The event binding name to use on `<ejs-pdfviewer>`
...
```html
<ejs-pdfviewer (annotationMove)="onAnnotationMove($event)"></ejs-pdfviewer>
```
...
```html
<ejs-pdfviewer (beforeAddFreeText)="onBeforeAddFreeText($event)"></ejs-pdfviewer>
```
...
```html
<ejs-pdfviewer (annotationSelect)="onAnnotationSelect($event)"></ejs-pdfviewer>
```
...
```html
<ejs-pdfviewer (annotationPropertiesChange)="onAnnotationPropertiesChange($event)"></ejs-pdfviewer>
```

...
```cshtml
@Html.EJS().PdfViewer("pdfviewer").EnableDownload(true).Render()
```
...
```cshtml
@Html.EJS().PdfViewer("pdfviewer").EnableDownload(true).DownloadFileName("myDocument").Render()
```
...
@Html.EJS().PdfViewer("pdfviewer").EnableDownload(true).EnableFormFields(true).Render()
